...
| `0.50·p·L` | `0.92·H` | 上升段 |
| `p·L` | `H` | **弧顶** |
| `p·L + 0.35·(L-p·L)` | `0.88·H` | 下降段 |
| `p·L + 0.70·(L-p·L)` | `0.50·H` | 下降段 |
| `p·L + 0.92·(L-p·L)` | `fall·H` | 落线姿态 |
| `L` | `0` | 第二焊点（C2） |

控制点经 `Part.BSplineCurve.interpolate()` 插值成平滑样条，因此曲线**严格穿过两个质心**、并**整体落在平分平面内**（实测端点误差 < 1e-7 mm）。
