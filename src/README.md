...
| `p·L` | `H` | **Loop peak** |
| `p·L + 0.35·(L-p·L)` | `0.88·H` | Descending section |
| `p·L + 0.70·(L-p·L)` | `0.50·H` | Descending section |
| `p·L + 0.92·(L-p·L)` | `fall·H` | Landing attitude |
| `L` | `0` | Second bond point (C2) |

The control points are interpolated by `Part.BSplineCurve.interpolate()` into a smooth spline, so the curve **passes exactly through both centroids** and **lies entirely inside the bisector plane** (measured endpoint error < 1e-7 mm).
